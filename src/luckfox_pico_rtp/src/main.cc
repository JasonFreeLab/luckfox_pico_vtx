...
int main(int argc, char *argv[])
{
    ...
    // 配置视频参数，设置编码方式为H.265
    VideoParameters params = { .width = VIDEO_WIGHT, .height = VIDEO_HEIGHT, .fps = 30, .codec = "video/x-h265" };
    EncoderParameters encoderParams = { .type = "rtph265pay", .codec = "video/x-h265" }; // 设置编码器参数为H.265

	gstreamer_push_init(&params, &encoderParams, 0);
